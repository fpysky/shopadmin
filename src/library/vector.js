const CAV = { FRONT: 0, BACK: 1, DOUBLE: 2, SVGNS: 'http://www.w3.org/2000/svg' }; CAV.Array = typeof Float32Array === 'function' ? Float32Array : Array; CAV.Utils = { isNumber: function(a) { return !isNaN(parseFloat(a)) && isFinite(a) } };
(function() {
  for (var a = 0, b = ['ms', 'moz', 'webkit', 'o'], c = 0; c < b.length && !window.requestAnimationFrame; ++c)window.requestAnimationFrame = window[b[c] + 'RequestAnimationFrame'], window.cancelAnimationFrame = window[b[c] + 'CancelAnimationFrame'] || window[b[c] + 'CancelRequestAnimationFrame']; if (!window.requestAnimationFrame)window.requestAnimationFrame = function(b) { var c = (new Date()).getTime(), f = Math.max(0, 16 - (c - a)), g = window.setTimeout(function() { b(c + f) }, f); a = c + f; return g }; if (!window.cancelAnimationFrame) {
...
CAV.Mesh.prototype.update = function(a, b) {
  var c, d, e, f, g; this.geometry.update(); if (b) {
    for (c = this.geometry.triangles.length - 1; c >= 0; c--) {
      d = this.geometry.triangles[c]; CAV.Vector4.set(d.color.rgba); for (e = a.length - 1; e >= 0; e--) {
        f = a[e], CAV.Vector3.subtractVectors(f.ray, f.position, d.centroid), CAV.Vector3.normalise(f.ray), g = CAV.Vector3.dot(d.normal, f.ray), this.side === CAV.FRONT ? g = Math.max(g, 0) : this.side === CAV.BACK ? g = Math.abs(Math.min(g, 0)) : this.side === CAV.DOUBLE && (g = Math.max(Math.abs(g), 0)), CAV.Vector4.multiplyVectors(this.material.slave.rgba,
...
CAV.CanvasRenderer.prototype.render = function(a) {
  CAV.Renderer.prototype.render.call(this, a); var b, c, d, e, f; this.clear(); this.context.lineJoin = 'round'; this.context.lineWidth = 1; for (b = a.meshes.length - 1; b >= 0; b--) {
    if (c = a.meshes[b], c.visible) {
      c.update(a.lights, true); for (d = c.geometry.triangles.length - 1; d >= 0; d--) {
        e = c.geometry.triangles[d], f = e.color.format(), this.context.beginPath(), this.context.moveTo(e.a.position[0], e.a.position[1]), this.context.lineTo(e.b.position[0], e.b.position[1]), this.context.lineTo(e.c.position[0],
