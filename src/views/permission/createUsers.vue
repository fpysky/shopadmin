...
<script>
import { getAdminRoles,adminers } from '@/api/adminer'
export default {
    name:'createUsers',
    data(){
        return {
    ...
    created(){
        if(this.$route.params.roles){
            this.roles = this.$route.params.roles
        }
        let adminer = this.$route.params.adminer
        if(adminer){
            this.isEdit = true
            this.ruleForm.id = adminer.id
            this.ruleForm.name = adminer.name
            this.ruleForm.account = adminer.account
            this.ruleForm.email = adminer.email
            this.ruleForm.avatar = process.env.BASE_API + adminer.avatar
            this.getAdminRoles(adminer.id)
        }
    ...
    methods:{
        ...
        getAdminRoles(id){
            getAdminRoles({id:id}).then(response => {
